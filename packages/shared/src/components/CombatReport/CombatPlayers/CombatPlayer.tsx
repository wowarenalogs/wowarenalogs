...
export function CombatPlayer(props: IProps) {
  ...
  // Heal-absorbs this player applied to enemies (e.g. a DK's Necrotic Wound). Denying an
  // enemy heal is offensive pressure, so we only surface heal-absorbs cast on hostile units
  // (self/friendly mechanics like Tyr's Deliverance are excluded). The base spellId/spellName
  // on these actions is the heal-absorb ability, so the damage compilers group them correctly.
