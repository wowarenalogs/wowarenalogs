...
interface IProps {
  ...
  /**
   * Every round of the solo shuffle `combat` belongs to. Supply along with `onRoundSelected`
   * to let the viewer switch rounds from inside the report.
   */
...
export const CombatReportInternal = ({ matchId, roundId }: { matchId: string; roundId?: string }) => {
  ...
  // When the round picker is on screen it already says which round this is.
