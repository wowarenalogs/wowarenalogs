...
// Removed createExportString import - will use placeholder for now
...
async function getOrCreateTalentBuild(
  ...
  if (!build) {
    // Generate export string - placeholder for now
    // TODO: Implement server-side talent export string generation
...
async function exportTalentDataToGCS(bracket: string, ratingRange: [number, number]) {
  ...
  for (const snapshot of snapshots) {
    ...
    groupedBySpec[specId].push({
      ...
      usageRate: 0, // Will be calculated client-side
    });
  }
...
export async function handler(_event: unknown, _context: unknown, callback: () => void) {
  console.log('refreshTalentStats started');

  try {
    for (const bracket of ALLOWED_BRACKETS) {
      for (const ratingRange of RATING_RANGES) {
        console.log(`Processing ${bracket} ${ratingRange[0]}-${ratingRange[1]}...`);
    ...
    console.log('refreshTalentStats completed successfully');
  } catch (error) {
    console.error('Error in refreshTalentStats:', error);
    throw error;
  } finally {
