...
export const segmentToCombat = () => {
  return pipe(
    map((segment: ICombatEventSegment): IArenaMatch | IMalformedCombatData | null => {
      // See the retail pipeline's segmentToCombat for why this is wrapped: an uncaught throw
      // here would propagate out of this map() and silently kill match detection for the rest
      // of the session, not just the one bad segment.
      try {
        return segmentToCombatInner(segment);
      } catch (e) {
...
function segmentToCombatInner(segment: ICombatEventSegment): IArenaMatch | IMalformedCombatData | null {
